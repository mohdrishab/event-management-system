...
async function enrichCertificateViews(rows: CertificateView[]): Promise<CertificateView[]> {
  ...
  // Query applications by student_id (not uid)
  const [appsResult, studentsResult] = await Promise.all([
    eventIds.length && uids.length
      ? supabase
          .from('applications')
          .select('student_id, event_id, event_name, event_type, start_date, end_date')
  ...
  // Build map using student_id
  const appByStudentEvent = new Map<string, any>();
  for (const a of appsResult.data || []) {
    appByStudentEvent.set(`${String(a.student_id)}::${String(a.event_id)}`, a);
  }
  const studentById = new Map<string, any>((studentsResult.data || []).map((s: any) => [String(s.id), s]));

  const enriched: CertificateView[] = [];
  for (const c of rows) {
    const app = c.eventId ? appByStudentEvent.get(`${c.uid}::${c.eventId}`) : null;
